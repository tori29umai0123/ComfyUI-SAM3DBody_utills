...
## SAM 3D Body: Render Human From Pose And Chara JSON ノード

`pose_json` と `chara_json` を入力に取り、3D 素体をレンダリングするノード。キャラ系設定 (PCA 体型 / ボーン長 / ブレンドシェイプ) は `chara_json` に切り出されているため、このノードに残るウィジェットは **カメラ系コントロール + 前のめり補正のみ** で構成は非常にシンプル。
...
- 体型は `chara_json` (Setting Chara JSON ノード または Character Editor から) で制御
...
| chara_json | `"{}"` | — | キャラ設定 JSON（Setting Chara JSON ノード または Character Editor の出力） |
...
## SAM 3D Body: Setting Chara JSON ノード

旧 Render ノードの**キャラ設定部分**だけを切り出した独立ノード。preset と PCA 9 軸の体型 / 4 部位のボーン長 / 顔・体ブレンドシェイプを全スライダー制御し、`chara_json` (STRING) を出力する。Render ノードや Export Rigged FBX ノード等の `chara_json` / `character_json` 入力に直結できる。

- 出力 JSON は `chara_settings_presets/*.json` と互換のスキーマ
- preset (autosave / female / male / chibi …) を選ぶと、その値が一括でスライダーに上書きされる
- **autosave**: ノード実行時 (preset = `reset` 以外) に `chara_settings_presets/autosave.json` が自動更新される。次回 ComfyUI 起動時はこの値がスライダー初期値になる
...
| 出力 | 説明 |
|---|---|
| chara_json | キャラ設定 JSON 文字列。`body_params` / `bone_lengths` / `blendshapes` の 3 ブロックを持つ、`chara_settings_presets/*.json` 互換の構造 |
...
シェイプの一覧は `presets/face_blendshapes.npz` から自動発見される — Blender で新しいシェイプキーを追加して npz を再生成すれば、再起動後に UI に現れる（コード変更不要）。UI 上の `bs_` プレフィックスは見やすさのためで、FBX 側のキー名・`chara_json` 側のキーではプレフィックスなし。
